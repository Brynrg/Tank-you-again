...
export class PerceptionSystem {
  sense(ai: TankState, world: WorldState): SensoryData {
    ...
    const visibleTanks = Array.from(world.tanks).filter(t => 
      visionSet.visibleTankIds.has(t.id) && t.id !== ai.id
    );
    ...
    const visibleMines = Array.from(world.mines).filter(m => 
  ...
  private calculateThreatLevel(ai: TankState, projectiles: ProjectileState[], enemies: TankState[]): number {
    let threat = 0;
    ...
    for (const proj of projectiles) {
      ...
      if (angleDiff < Math.PI / 4) { // Projectiles heading toward AI
  ...
  private decideExpert(ai: TankState, perception: SensoryData, action: AIAction): void {
    ...
    if (perception.visibleTanks.length > 1 && Math.random() > 0.6) {
      ...
      if (weakestEnemy) {
        ...
        if (Math.hypot(weakestEnemy.x - ai.x, weakestEnemy.y - ai.y) < 300 && ai.ammo.missiles > 0) {
          action.fire = {
            weapon: ProjectileKind.MISSILE,
            aim: targetAngle
          };
