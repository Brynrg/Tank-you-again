...
console.log('   Easy AI:', `${easyAI.getTank().name} (${easyAI.getTank().team}, ${easyAI.getDifficulty()})`);
console.log('   Medium AI:', `${initialAI.getTank().name} (${initialAI.getTank().team}, ${initialAI.getDifficulty()})`);
console.log('   Hard AI:', `${hardAI.getTank().name} (${hardAI.getTank().team}, ${hardAI.getDifficulty()})`);
console.log('   Expert AI:', `${expertAI.getTank().name} (${expertAI.getTank().team}, ${expertAI.getDifficulty()})`);
...
tanks.forEach((tank, index) => {
  ...
  console.log(`     Ammo: missiles=${tank.ammo.missiles}, mines=${tank.ammo.mines}, teleports=${tank.ammo.teleports}, shields=${tank.ammo.shields}, radar=${tank.ammo.radar}`);
...
finalTanks.forEach((tank, index) => {
  console.log(`   ${index + 1}. ${tank.name} - Team: ${tank.team}, Difficulty: ${tank.rank}, Position: (${tank.x.toFixed(1)}, ${tank.y.toFixed(1)})`);
});
...
finalTanks.forEach(tank => {
  console.log(`   ${tank.name}: matches "AI-{difficulty}-{random}" pattern? ${tank.name.startsWith('AI-')}`);
});
...
console.log(`✅ Difficulty levels: ${Array.from(new Set(finalTanks.map(t => t.rank))).join(', ')}`);
console.log(`✅ Tank naming: ${finalTanks.every(t => t.name.startsWith('AI-')) ? 'Correct' : 'Incorrect'}`);
