...
export class RoomLoop {
  ...
  private buildSnapshotFor(
    ...
    const projArr = this.projectiles.valuesArray();
    ...
    const pickArr = this.pickups.valuesArray();
  ...
  private collectPickups(): void {
    ...
    const collectTanksArr = this.tanks.valuesArray();
    for (let i = 0; i < collectTanksArr.length; i++) {
      const t = collectTanksArr[i]!;
